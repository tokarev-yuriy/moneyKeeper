<?php

return array(

	/*
	|--------------------------------------------------------------------------
	| MoneyKeep Language Lines
	|--------------------------------------------------------------------------
	|
	| The following language lines contain menu's, type's and other money keep labels.
	|
	*/

    "title" => "Money",
    "total" => "Total",
    "spends" => "Spends",
    "incomes" => "Incomes",
    "transfers" => "Transfers",
    "wallets" => "Wallets",
    "categories" => "Categories",
    "logout" => "Logout",
    "login" => "Login",
    "email" => "E-Mail Address",
    "password" => "Password",
    "remember" => "Remember Me",
    "register" => "Register",
    "name" => "Name",
    "confirm_password" => "Confirm Password",
    "sort" => "Sort",
    "start" => "Start",
    "save" => "Save",
    "type" => "Type",
    "date" => "Date",
    "summ" => "Summ",
    "icon" => "Icon",
    "color" => "Color",
    "category" => "Category",
    "comment" => "Comment",
    "src_wallet" => "Source Wallet",
    "dest_wallet" => "Destination Wallet",
    "wallet" => "Wallet",
    "wrong_pass" => "Wrong Password or Login",
    
    "items" => "Items",
    "add_item" => "Add item",
    "edit_item" => "Edit item",
    "delete_item" => "Delete item",
    
    "categories" => "Categories",
    "add_category" => "Add category",
    "edit_category" => "Edit category",
    "delete_category" => "Delete category",
    "all_categories" => "All categories",
    
    "wallets" => "Wallets",
    "add_wallet" => "Add wallet",
    "edit_wallet" => "Edit wallet",
    "delete_wallet" => "Delete wallet",
    
    "operations" => "Operations",
    "add_operation" => "Add operation",
    "edit_operation" => "Edit operation",
    "delete_operation" => "Delete operation",
    
    "spends" => "Spends",
    "add_spend" => "Add spend",
    "edit_spend" => "Edit spend",
    "delete_spend" => "Delete spend",
    
    "incomes" => "Incomes",
    "add_income" => "Add income",
    "edit_income" => "Edit income",
    "delete_income" => "Delete income",
    
    "transfers" => "Transfers",
    "add_transfer" => "Add transfer",
    "edit_transfer" => "Edit transfer",
    "delete_transfer" => "Delete transfer",
    
    "spend" => "Spend",
    "income" => "Income",
    "transfer" => "Transfer",
    "any" => "Any",
    
    "statistics" => "Statistics",
    "stat_monthly" => "Statistics monthly",
    "stat_monthly_avg" => "Statistics monthly average",
    "stat_year" => "Statistics year",
    "stat_year_avg" => "Statistics year average",
    "per_month" => "per month",
    "per_year" => "per year",
    "in_month" => "in month",
    "in_year" => "in year",
    "no_data" => "There is no information",
    "hide_pie_chart" => "Hide Pie Chart",
    "show_pie_chart" => "Show Pie Chart",

    "plans" => "Plans",
    "add_plan" => "Add plan",
    "edit_plan" => "Edit plan",
    "delete_plan" => "Delete plan",
    
    "import" => 'Import',
    "import_file" => 'Import file',
    "import_profile" => 'Import Profile',
    "import_profiles" => 'Import Profiles',
    "round" => 'Round',
    "add_import_profile" => 'Add Import Profile',
    "edit_import_profile" => 'Edit Import Profile',
    "delete_import_profile" => 'Delete Import Profile',
    "encoding" => 'Encoding',
    "start_row" => 'Number of the first line with data',
    "control_row" => 'Number of the line with control text',
    "control_string" => 'Control text',
    "date_col" => 'Date col. number',
    "summ_col" => 'Summ col. number',
    "category_col" => 'Category col. number',
    "desc_col" => 'Description col. number',
    "import_file_wrong" => 'Incorrect import file',
    "import_profile_category_rules" => 'Category rules',
    "import_profile_category_rules_help" => 'Substrings of descriptions',
    "import_check_items" => 'Please check import data',
    
    "not_selected" => 'Not selected',
    
    "wallet_group_others" => "Others",
    "wallet_group" => "Group",
    "wallets_groups" => "Groups",
    "add_wallets_group" => "Add",
    
    "sync" => "Sync",
    "integration" => "Integration",
    "integrations" => "Integrations",
    "clientId" => "clientId",
    "clientSecret" => "clientSecret",
    "refreshToken" => "refreshToken",
    "edit_integration" => "Edit integration",
    "add_integration" => "Add integration",
    "delete_integration" => "Delete integration",
    
);
