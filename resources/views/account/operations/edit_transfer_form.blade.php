{!! Form::open(array('url' => ((isset($obItem) && $obItem->id)?$paths['update'].'/'.$obItem->id:$paths['add']))) !!}
  ...
  <div class="form-row">
     <div class="col-6 form-group">
         ...
         @php
           echo \App\MoneyKeeper\Helpers\Form::dropdownSelect('category_id', \App\MoneyKeeper\Models\Operation::getTypeCategoriesWithIcons('transfer'), Input::get('category_id', isset($obItem)?$obItem->category_id:''), (isset($errors) && $errors->has('category_id') ? 'is-invalid' : ''));
         @endphp
    ...
    <div class="form-row">
     <div class="col-6 form-group">
         <label for="wallet_from_id" class="mb-0">{{ trans('mkeep.src_wallet') }}</label>
          @php
            echo \App\MoneyKeeper\Helpers\Form::dropdownSelect('wallet_from_id', \App\MoneyKeeper\Models\Operation::getWalletsWithIcons(), Input::get('wallet_from_id', isset($obItem)?$obItem->wallet_from_id:Session::get('wallet_from_id')), (isset($errors) && $errors->has('wallet_from_id') ? 'is-invalid' : ''));
          @endphp
           <span class="invalid-feedback">
     ...
     <div class="col-6 form-group">
         <label for="wallet_to_id" class="mb-0">{{ trans('mkeep.dest_wallet') }}</label>
          @php
     ...
     <div class="row form-group">
        ...
        <div class="col-6 pr-0">
            <button id="btn-type-drop" type="button" class="btn btn-seccondary pull-right dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
              <i class="fa fa-exchange"></i>&nbsp;&nbsp;{{ trans('mkeep.add_transfer') }}
            </button>
            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
              ...
              <a class="dropdown-item text-success" data-btn-type="edit" href="/account/operations/income/{{ ((isset($obItem) && $obItem->id)?'update/'.$obItem->id:'add') }}"><i class="fa fa-long-arrow-right"></i>&nbsp;&nbsp;{{ trans('mkeep.add_income') }}</a>
            </div>
        </div>
     </div>
